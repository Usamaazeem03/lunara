...
function DashboardStatCard({
  ...
  return (
    <div className="relative overflow-hidden border border-[#2d2620]/20 bg-white/70 p-3 transition-all duration-300 hover:bg-white/90 hover:shadow-sm sm:p-4">
      {showAccent && (
        <div className="absolute -top-6 -right-6 h-16 w-16 rounded-full bg-[#2d2620]/5" />
      )}

      <div className="flex justify-between gap-2">
        <div className="w-full">
          {label && (
            <p className="text-xs tracking-widest text-[#5f544b]/80 uppercase">
              {label}
            </p>
          )}
        ...
        {icon && (
          <div className="flex h-10 w-12 items-center justify-center rounded-full border border-[#2d2620]/20 bg-[#f3efe9]">
            {isValidElement(icon) ? (
              icon
            ) : (
