  ...
  const selectedClient = useMemo(() => {
    ...
    return (
      mergedClients.find((client) => {
        if (
          routeStateClientId &&
          String(client.id) === String(routeStateClientId)
        ) {
          return true;
        }
        ...
        if (
          routeStateClientId &&
          String(client.auth_id ?? "") === String(routeStateClientId)
        ) {
          return true;
        }

        return (
          normalizedClientSlug && client.routeSlug === normalizedClientSlug
        );
      }) ?? null
    );
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-[0.9fr_1.4fr]">
            <aside className="border-ink/20 flex flex-col gap-4 border-2 bg-white/90 p-5">
              ...
              <div className="border-ink/10 bg-cream/50 border p-4">
                ...
                <p className="mt-1 text-2xl font-semibold">GBP {totalSpent}</p>
