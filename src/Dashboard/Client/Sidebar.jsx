      ...
      <div className="flex items-center gap-3">
        {onProfileClick ? (
          <button
            ...
            className={`${avatarClasses} transition hover:scale-[1.02] hover:border-[#2d2620]/40 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#2d2620]/40`}
          >
            <img
              src={profileImg}
              alt={profileAlt}
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-4 rounded-full px-3 py-2 text-left text-sm uppercase tracking-widest transition ${
        active
          ? "bg-[#2d2620] text-[#f3efe9]"
          : "text-[#2d2620] hover:bg-[#2d2620]/10"
      }`}
    >
      <span
        className={`flex h-9 w-9 items-center justify-center rounded-full border border-[#2d2620]/20 ${
          active ? "bg-[#2d2620]" : "bg-white/70"
        }`}
      >
        <img
          src={icon}
